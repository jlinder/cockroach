...
func (tc *ToxiCluster) Measure(ctx context.Context, fromNode int, stmt string) time.Duration {
	...
	args := []string{"time", "-p", "./cockroach", "sql"}
	if insecure {
		args = append(args, "--insecure")
	} else {
		args = append(args, "--certs-dir", "certs")
	}
	args = append(args,  "--port", strconv.Itoa(port), "-e", "'"+stmt+"'")
	b, err := tc.cluster.RunWithBuffer(ctx, tc.cluster.l, tc.cluster.Node(fromNode), args...)
